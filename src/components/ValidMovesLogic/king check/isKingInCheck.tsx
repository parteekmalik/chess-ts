...
import { knightKing, pieceOnLoc,rookBishop } from "../pieceLogic";
...
export const iskingInCheck = (boardData: boardData_Type): boolean => {
  ...
  let moves = rookBishop(boardData, { row: kingPos.row, col: kingPos.col, pieceType: "Rook" });
  for (let i = 0; i < moves.length; i++){
    const res = pieceOnLoc(boardData,{row:moves[i].row,col:moves[i].col});
    if (["OQ","OR"].includes(res)) return true;
  }

  moves = rookBishop(boardData, { row: kingPos.row, col: kingPos.col, pieceType: "Bishop" });
  ...
  const pawnRow = kingPos.row + (turn === "white" ? -1 : 1);
  const resL = pieceOnLoc(boardData, { row: pawnRow, col: kingPos.col - 1 });
  const resR = pieceOnLoc(boardData, { row: pawnRow, col: kingPos.col + 1 });
  if (resL === "OP" || resR === "OP") return true;
