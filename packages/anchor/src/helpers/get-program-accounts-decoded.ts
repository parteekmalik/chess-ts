import type { Account, AccountInfoBase, AccountInfoWithBase64EncodedData, Decoder, MaybeEncodedAccount, SolanaClient } from "gill";
import { decodeAccount, parseBase64RpcAccount } from "gill";

import type { GetProgramAccountsConfig } from "./get-program-accounts";
...
export async function getProgramAccountsDecoded<T extends object>(
  ...
  const encodedAccounts: MaybeEncodedAccount[] = programAccounts.map((item) => {
    const account = parseBase64RpcAccount(item.pubkey, item.account as AccountInfoBase & AccountInfoWithBase64EncodedData);
    return {
      ...account,
