import type { Account, Blockhash, Instruction, KeyPairSigner, ProgramDerivedAddress, SolanaError } from "gill";
import {
  createSolanaClient,
  createTransaction,
...
import type { ChessMatch, Profile, Registry } from "../src";
import {
  fetchChessMatch,
  fetchProfile,
  fetchRegistry,
...
describe("web3", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      ...
      whitePlayer,
      "balance: ",
      (Number((await rpc.getBalance(whitePlayer.address).send()).value) / LAMPORTS_PER_SOL).toFixed(2),
    );
    console.log(
      "blackPlayer: ",
      blackPlayer,
      "balance: ",
      (Number((await rpc.getBalance(blackPlayer.address).send()).value) / LAMPORTS_PER_SOL).toFixed(2),
    );
  ...
  it("Make move", async () => {
    ...
    let opponentProfile =
      chessMatch.data.white.__option === "Some" && chessMatch.data.white.value === whiteProfile.address ? blackProfile : whiteProfile;
