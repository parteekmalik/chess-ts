...
async function handleExpoSigninCallback(req: NextRequest, redirectURL: string) {
  ...
  const setCookie = authResponse.headers.getSetCookie().find((cookie) => AUTH_COOKIE_PATTERN.test(cookie));
  const match = setCookie?.match(AUTH_COOKIE_PATTERN)?.[1];

  if (!match) throw new Error("Unable to find session cookie: " + JSON.stringify(authResponse.headers.getSetCookie()));
...
export const POST = async (_req: NextRequest, props: { params: Promise<{ nextauth: string[] }> }) => {
...
export const GET = async (_req: NextRequest, props: { params: Promise<{ nextauth: string[] }> }) => {
  // First step must be to correct the request URL.
  const req = rewriteRequestUrlInDevelopment(_req);
