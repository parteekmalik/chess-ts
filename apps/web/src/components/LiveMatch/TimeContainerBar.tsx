...
export const TimerContainer = ({ variant, isTurn, time, userId }: { variant: "white" | "black"; time: number; userId?: string; isTurn: boolean }) => {
  ...
  const prevTime = useRef(Infinity);
  ...
  useLayoutEffect(() => {
    if (isTurn && gameData?.status === MatchStatus.Active) {
      ...
      worker.onmessage = (e) => {
        ...
        if (Math.abs(prevTime.current - newTime) > 998) {
    ...
    <Card className="border-0 bg-inherit">
      <CardContent className="flex w-full justify-between p-0">
        ...
        <TimerComponent time={liveTimeLeft} variant={variant} isTurn={isTurn}>
          {isTurn && <ClockSvg rotation={rotation} />}
        </TimerComponent>
      </CardContent>
    </Card>
  );
...
const TimerComponent = ({ time, variant, isTurn, children }: { children: ReactNode; time: number; variant: "white" | "black"; isTurn: boolean }) => {
...
const ClockSvg = ({ rotation }: { rotation: number }) => {
  return (
    <div>
