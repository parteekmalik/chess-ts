...
# Solana Movie CRUD Application

A complete CRUD (Create, Read, Update, Delete) application built on Solana blockchain for managing movie records.

## Features

- **Create**: Add new movies with title, description, and rating
- **Read**: Retrieve movie information from the blockchain
- **Update**: Modify existing movie details (only by the owner)
- **Delete**: Remove movies from the blockchain (only by the owner)
- **Ownership**: Each movie is owned by the wallet that created it
...
solana/
├── src/
│   ├── lib.rs          # Main program logic with CRUD operations
│   └── state.rs        # Data structures and instruction definitions
├── client/
│   ├── movie-client.ts # TypeScript client for interacting with the program
...
solana program deploy target/deploy/chess_solana.so
```
...
const client = new MovieClient(connection, programId, payer);

// Request airdrop for testing
await client.requestAirdrop(2);

// Create a movie
const moviePubkey = await client.createMovie(
  "The Matrix",
...
// Read movie details
const movie = await client.readMovie(moviePubkey);

// Update movie
await client.updateMovie(moviePubkey, "The Matrix Reloaded", undefined, 8);

// Delete movie
await client.deleteMovie(moviePubkey);
```

## Program Instructions

The Solana program supports four main instructions:

### 1. CreateMovie

- **Accounts**:
  - `movie_account`: New account to store movie data
  - `user_account`: Signer creating the movie
  - `rent`: System rent account
...
- **Accounts**:
  - `movie_account`: Account containing movie data
- **Data**: None (just reads existing data)

### 3. UpdateMovie

- **Accounts**:
  - `movie_account`: Account to update
  - `user_account`: Signer (must be movie owner)
- **Data**: Optional title, description, rating

### 4. DeleteMovie

- **Accounts**:
  - `movie_account`: Account to delete
  - `user_account`: Signer (must be movie owner)
  - `rent`: System rent account
  - `system_program`: System program for account cleanup
...
pub struct MovieAccountState {
    ...
    pub title: String,             // 4 bytes + string length
    pub description: String,       // 4 bytes + string length
...
- **Ownership Verification**: Only movie owners can update or delete their movies
- **Signature Validation**: All write operations require valid signatures
- **Account Validation**: Proper account ownership and initialization checks
- **Rent Management**: Automatic rent calculation and refunds
...
- Ensure proper account setup and funding

## Advanced Features

- **Batch Operations**: Process multiple movies in a single transaction
- **Search and Filter**: Implement on-chain search capabilities
- **Access Control**: Add role-based permissions
- **Event Logging**: Enhanced transaction logging and event emission
